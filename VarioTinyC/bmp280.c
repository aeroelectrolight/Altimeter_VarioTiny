...
uint8_t bmp280_init(void)
{
	...
	bmp280_set_ctrl(2, 5, 3); // T oversample x2, P over x16, normal mode
...
/*
float bmp280_getaltitude(float seaLevel) {
	float alt;
	float press = _bmp280_pres;
	press /= 100;
	alt = 44330 * ( 1.0 - pow(press/seaLevel, 0.1903));
...
float bmp280_getaltitude(){
	...
	if ( _bmp280_pres > 95461) {
		...
		} else if ( _bmp280_pres > 701120000) {
		alt = 25000000 + ( 746860000 - _bmp280_pres ) *  0.109313511 ;
		} else if ( _bmp280_pres > 657680000) {
		alt = 30000000 + ( 701120000 - _bmp280_pres ) *  0.115101289 ;
		} else if ( _bmp280_pres > 616450000) {
		alt = 35000000 + ( 657680000 - _bmp280_pres ) *  0.121270919 ;
		} else if ( _bmp280_pres > 577330000) {
		alt = 40000000 + ( 616450000 - _bmp280_pres ) *  0.127811861 ;
		} else {    alt = 4500 + ( 57733 - _bmp280_pres ) *  0.134843581 ;*/
	}
	return alt*100;
}